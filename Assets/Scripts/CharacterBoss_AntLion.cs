...
public class CharacterBoss_AntLion : CharacterBoss
{
    private eAntLionState m_State = eAntLionState.Standing;
    ...
    IEnumerator Action()
    {
        ...
        while (!m_IsDead)
        {
            m_State = (eAntLionState)ReturnActionNum();
            ...
            switch (m_State)
            {
                case eAntLionState.Standing:
                    PlayAnimation(eCharacterAnimState.Standing, true);
                    break;

                case eAntLionState.Attack:
                    StartCoroutine(Attack());
                    break;

                case eAntLionState.Shield:
                    StartCoroutine(Skill_Shield());
                    break;

                case eAntLionState.Smash:
                    StartCoroutine(Skill_Smash());
                    break;

                case eAntLionState.Dead:
    ...
    public override void AfterDead()
    {
        ...
        {
            if (MainController.Instance.UserInfo.GetTutorialStoryClear(eStoryState.Stage4EndStroy) == false)
            {
                TutorialStorySystem.Instance.StartSpeach(eStoryState.Stage4EndStroy);
            }
            else
            {
